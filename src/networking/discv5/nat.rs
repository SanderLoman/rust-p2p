...
impl Nat {
    pub async fn new() -> Result<Nat, Box<dyn std::error::Error>> {
    ...
    pub fn get_public_ip(&self) -> Result<Ipv4Addr, Box<dyn std::error::Error>> {
        let ip: Ipv4Addr = self.gateway.get_external_ip()?;
        Ok(ip)
    }
    ...
    pub fn remove_port_mapping(
        ...
        self.gateway.remove_port(protocol, external_port)?;
        Ok(())
    }
}
