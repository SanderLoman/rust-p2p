...
pub async fn process_discovered_peers() -> Result<Vec<String>, Box<dyn Error>> {
    ...
    for addr_str in &connected_peers {
        ...
        if let Some(peer_id) = addr.iter().find_map(|proto| match proto {
            ...
            // Extract the IP address from the Multiaddr and convert it to an IpAddr
            let ip = addr
                .iter()
                ...
                .expect("No IP address found in Multiaddr");

            // Extract the port from the Multiaddr
            let port = addr
                .iter()
                .find_map(|proto| match proto {
                    libp2p::core::multiaddr::Protocol::Tcp(port) => Some(port),
                    libp2p::core::multiaddr::Protocol::Udp(port) => Some(port),
                    _ => None,
                })
                .expect("No port found in Multiaddr");

            // Get the Ethereum block number
            match get_beacon_chain_block_number(ip, port).await {
                Ok(block_number) => {
                    println!(
                        "{}",
                        LogEntry {
                            ...
                            level: LogLevel::Info,
                            message: format!(
                                "Current block number for peer {:?}: {}",
                                peer_id, block_number
                            ),
                        }
                    );
                }
                Err(e) => {
                    println!(
                        ...
                        LogEntry {
                            ...
                            level: LogLevel::Error,
                            message: format!(
                                "Failed to get block number for peer {:?}: {}",
                                peer_id, e
                            ),
                        }
                    );
    ...
    // /ip4/83.128.33.146/tcp/53998/p2p/16Uiu2HAm3CHQXGJokLWodbDocko58tCdgotxYcR6BuXyLKcuobUR
    // /ip4/83.128.33.146/udp/53998/p2p/16Uiu2HAm3CHQXGJokLWodbDocko58tCdgotxYcR6BuXyLKcuobUR
